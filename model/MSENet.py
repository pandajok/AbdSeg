...
class MSENet(nn.Module):
    ...
    def __init__(self, cfg=None):
        ...
        encoder_conv_block = ResFourLayerConvBlock1
        decoder_conv_block = ResTwoLayerConvBlock1
        context_block= ResFourLayerConvBlock
        ...
        self.conv2_2 = decoder_conv_block(num_channel[2] + num_channel[3] + num_channel[4], num_channel[2],
                                          num_channel[2])
        self.conv1_3 = decoder_conv_block(num_channel[1] + num_channel[2] + num_channel[3],
                                          num_channel[1], num_channel[1])
        self.conv0_4 = decoder_conv_block(
            num_channel[0] + num_channel[1], num_channel[0],
            num_channel[0])
        ...
        self.conv2_2_1 = decoder_conv_block(num_channel[2] + num_channel[3], num_channel[2],
                                            num_channel[2])
        self.conv1_3_1 = decoder_conv_block(num_channel[1] + num_channel[2],
                                            num_channel[1], num_channel[1])
        self.conv0_4_1 = decoder_conv_block(
            num_channel[0] + num_channel[1], num_channel[0],
            num_channel[0])
        self.conv1 = nn.Conv3d(num_channel[1], num_channel[1], 1, 1, 0)
        self.conv2 = nn.Conv3d(num_channel[1], num_channel[1], 1, 1, 0)
        self.conv3 = nn.Conv3d(num_channel[0], num_channel[0], 1, 1, 0)
        self.conv4 = nn.Conv3d(num_channel[0], num_channel[0], 1, 1, 0)
        self.relu = nn.ReLU(inplace=True)
        self.context_block = context_block(num_channel[4],num_channel[4],num_channel[4])
    ...
    def forward(self, x):
        ...
        x2_0 = self.conv2_2(torch.cat([x2_0, self.up(x3_0), self.up(self.up(x4_0))], 1))
        x1_0 = self.conv1_3(
            torch.cat([x1_0, self.up(x2_0), self.up(self.up(x3_0))], 1))
        x = self.conv0_4(torch.cat([x, self.up(x1_0)], 1))

        x6_0 = self.conv2_2_1(torch.cat([self.up(x3_0), x2_0], 1))
        x7_0 = self.conv1_3_1(torch.cat([self.up(x6_0), x1_0], 1))
        x7_0 = self.conv1(x7_0)
        x7_0 = self.relu(x7_0)
        x7_0 = x7_0 * x1_0
        x7_0 = self.conv2(x7_0)
        x8_0 = self.conv0_4_1(torch.cat([self.up(x7_0), x], 1))
        x8_0 = self.conv3(x8_0)
        x8_0 = self.relu(x8_0)
        x8_0 = x8_0 * x
        x8_0 = self.conv4(x8_0)
        x = self.final(x8_0)
